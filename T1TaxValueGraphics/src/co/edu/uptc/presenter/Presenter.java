...
//		persistence.readDates(simulatorTest.getBrands());
		...
		viewTest = new View(this, null, "Matriculado en Boyaca", "Pago Oportuno", "Vehiculo Hibrido o Electrico", brands());
	}
...
//			System.out.println(viewTest.brand());
	...
	public Model searchModel(String nameBrand, String nameLine, String nameModel) {
		Model model = null;
		...
		if(brand != null) {
			...
			line = simulatorTest.searchLine(brand, line);
			if(line != null) {
				model = simulatorTest.searchValue(brand, line, Integer.parseInt(nameModel));
				if(model == null) {
					viewTest.showMessage("El modelo no existe en el sistema");
				}
			} else {
